...
const Navbar = () => {
  ...
  return (
    <div className="fixed top-0 inset-x-0 z-50 flex justify-center px-4 pt-4 pointer-events-none">
      <nav
        className={`pointer-events-auto w-full max-w-7xl transition-all duration-500 ease-out rounded-full border border-white/40 backdrop-blur-xl ${
          scrolled
            ? "bg-white/80 shadow-[0_8px_30px_rgb(0,0,0,0.04)] py-3"
            : "bg-white/60 shadow-sm py-4"
        }`}
      >
            ...
            <div className="flex items-center gap-8 mr-4">
              {navLinks.map((link) => {
                ...
                return (
                  ...
                  >
                    <span className={isActive ? "text-teal-800 font-bold" : ""}>
                      {link.name}
                    </span>
                    <span
      ...
      >
        <div className="bg-white/95 backdrop-blur-xl border border-white/50 shadow-2xl rounded-3xl p-6 flex flex-col gap-4">
          ...
          {/* 🟢 2. Place InstallPWA inside the mobile menu so visitors can install directly from their phone browser */}
          <div className="mb-2">
            <InstallPWA />
          </div>
