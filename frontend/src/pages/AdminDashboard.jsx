...
const AdminDashboard = () => {
  ...
  const navItems = [
    ...
    { id: "graphs", label: "Registrations", icon: LineChart },
  ...
  return (
    <>
      ...
      <div className="flex h-screen bg-slate-50 font-sans text-slate-800 overflow-hidden selection:bg-indigo-200 relative">
          ...
          <nav className="md:hidden fixed bottom-6 left-4 right-4 bg-white/90 backdrop-blur-2xl border border-white/50 shadow-[0_20px_60px_rgba(0,0,0,0.12)] z-50 px-2 pb-safe pt-2 rounded-3xl">
            <div className="flex justify-around items-center h-16">
              {navItems.map((item) => (
                ...
                >
                  <div
                    className={`${activeTab === item.id ? "bg-indigo-50 shadow-inner border border-indigo-100/50 p-1.5 rounded-xl" : ""}`}
                  >
                    <item.icon
                      size={22}
...
const SidebarItem = ({ icon, label, isActive, onClick }) => (
  <button
    ...
    className={`w-full flex items-center gap-4 py-4 px-5 rounded-2xl transition-all duration-300 font-bold group ${
      isActive
        ? "bg-gradient-to-r from-indigo-500 to-indigo-400 text-white shadow-[0_0_20px_rgba(99,102,241,0.3)] translate-x-2"
